...
s_rd = s_echo.*exp(-1j*2*pi*fnc.*(eta.'*ones(1,Nrg))); 	% 數據搬移,課本找不到(◕‿◕)?
S_rd = fft(s_rd,NFFT_a,1);                              % 方位向FFT變化,到距離多普勒域(Azimuth FFT)(7.16) 類似(6.9)

% 定義Aziumth向的頻率域
feta = fnc + fftshift( -NFFT_a/2 : NFFT_a/2-1 )*( Fa/NFFT_a );   % 方位频率轴
feta_mtx = feta.'*ones(1,Nrg);                                   % 方位频率轴矩阵，大小：Naz*Nrg

D_feta_Vr = sqrt(1-lamda^2.*(feta.').^2./(4*Vr^2));       % 大斜視角下的徒動因子,行向量 (7.17)
D_feta_Vr_mtx = D_feta_Vr*ones(1,Nrg);                    % 形成矩阵，大小：Nrg*Naz

feta_ref = fnc;        	                                  % 参考目标的多普勒中心频率
D_feta_ref_Vr = sqrt(1-lamda^2*feta_ref^2/(4*Vr^2));      % 参考频率fn_ref处的徙动因子，是常数。

R_ref = R0;                                               % 参考目标选在场景中心，其最近斜距为 R_ref
K_src = 2*Vr^2*f0^3.*D_feta_Vr.^3./(c*R_ref*(feta.').^2); % 列向量，使用R_ref处的值 
K_src_mtx = K_src*ones(1,Nrg);                            % 形成矩阵
Km = Kr./(1-Kr./K_src_mtx);                               % 矩阵，这是变换到距离多普勒域的距离调频率(7.18)

% 下面生成 变标方程 s_sc (7.29)
s_sc = exp(1j*pi.*Km.*(D_feta_ref_Vr./D_feta_Vr_mtx-1).*(tau_mtx-2*R_ref./(c.*D_feta_Vr_mtx)).^2);

% 下面将距离多普勒域的信号与变标方程相乘，实现“补余RCMC”
S_RD_1 = S_rd.*s_sc;            % 相位相乘，实现“补余RCMC” (7.31)
...
%% 
% --------------------------------------------------------------------
% 变换到二维频域，进行“距离压缩，SRC，一致RCMC” (7.32)
% --------------------------------------------------------------------
S_2df_1 = fft(S_RD_1,NFFT_r,2);         % 进行距离向FFT，变换到二维频域。距离零频在两端

% 完成距离压缩，SRC，一致RCMC这三者相位补偿的滤波器为：
H1 = exp(1j*pi.*D_feta_Vr_mtx./(D_feta_ref_Vr.*Km).*fr_mtx.^2)...
    .*exp(1j*4*pi/c.*(1./D_feta_Vr_mtx-1/D_feta_ref_Vr).*R_ref.*fr_mtx);
% 上面的H1距离零频在中心
...
% --------------------------------------------------------------------
R0_RCMC = (c/2).*tau;   % 随距离线变化的R0，记为R0_RCMC，用来计算方位MF。 (7.25)
% 生成方位向匹配滤波器
Haz = exp(1j*4*pi.*(D_feta_Vr*R0_RCMC).*f0./c);       % 方位MF

% 附加相位校正项
H2 = exp(-1j*4*pi.*Km./(c^2).*(1-D_feta_Vr_mtx./D_feta_ref_Vr)...
    .*((1./D_feta_Vr)*R0_RCMC-R_ref./D_feta_Vr_mtx).^2); 	% 附加相位校正项
...
% 的距离单元（即 R0/D_feta_ref_Vr ），因此对应的目标1的y轴位置如下，为 tg_1_y ：
tg_1_y = round( (Nrg/2+1) + 2*(R0/D_feta_ref_Vr-R0)/c*Fr );
...
tg_3_x = fix(tg_3_x);
tg_3_y = tg_2_y + 2*(delta_R2/D_feta_ref_Vr)/c*Fr;
